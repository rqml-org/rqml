...
| **[`@rqml/mcp`](https://rqml.org/docs/tooling/mcp)** | `npx @rqml/mcp` | Twelve [MCP](https://modelcontextprotocol.io) tools for coding agents (`rqml_show`, `rqml_overview`, `rqml_matrix`, `rqml_approve`, `rqml_gate`, `rqml_check`, …) — reads specs by path, writes only on explicit intent |
